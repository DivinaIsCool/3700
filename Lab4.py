...
hist1, bound1 = np.histogram(R1, bins=15)
hist2, bound2 = np.histogram(R2, bins=15)
hist12, bound12 = np.histogram(R12, bins=25)
center1=np.array([])
...
plt.plot(rxdata, rydata, 'o', label='Data', color='black', zorder=3)
error1=hist1**.5
plt.errorbar(center1,hist1,yerr=error1, linestyle='', marker='o', color='black', zorder=2)
plt.plot(xdata, fit_y*100/15, '-', label='Fit', color='orange', zorder=1)
plt.legend()
plt.xlabel('R1 (Ω)')
...
rydatainterp = np.interp(rxdata, xdata, fit_y*100/15)
chi1=chi(rydata,rydatainterp,fit_C)
...
plt.plot(rxdata, rydata, 'o', label='Data', color='black', zorder=3)
error2=hist2**.5
plt.errorbar(center2,hist2,yerr=error2, linestyle='', marker='o', color='black', zorder=2)
plt.plot(xdata, fit_y*100/15, '-', label='Fit', color='orange', zorder=1)
plt.legend()
plt.xlabel('R2 (Ω)')
plt.ylabel('Occurances')
...
rydatainterp = np.interp(rxdata, xdata, fit_y*100/15)
chi2=chi(rydata,rydatainterp,fit_C)
...
hist1, bound1 = np.histogram(R1, bins=15)
hist2, bound2 = np.histogram(R2, bins=15)
hist12, bound12 = np.histogram(R12, bins=15)
center1=np.array([])
for i, o in enumerate(bound1):
    if i < (len(bound1)-1):
        center = (bound1[i] + bound1[i+1])/2
...
plt.plot(rxdata, rydata, 'o', label='Data', color='black', zorder=3)
error12=hist12**.5
plt.errorbar(center12,hist12,yerr=error12, linestyle='', marker='o', color='black', zorder=2)
plt.plot(xdata, fit_y*250/15, '-', label='Fit', color='orange', zorder=1)
plt.legend()
plt.xlabel('R1 + R2 (Ω)')
plt.ylabel('Occurances')
plt.show()
...
rydatainterp = np.interp(rxdata, xdata, fit_y*250/15)
chi12=chi(rydata,rydatainterp,fit_C)
